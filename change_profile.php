...
if ($result->num_rows > 0) {
    while($row = mysqli_fetch_array($result)) {
        if($row['user_pw'] == $current_pw){
            $update = "UPDATE login_info SET user_pw = '$new_pw' WHERE user_id = '$username'"; /*php mysql update W3School*/
        ...
        if($email !=""){
            $update = "UPDATE login_info SET user_email = '$email' WHERE user_id = '$username'";/*php mysql update W3School*/
            mysqli_query($db, $update) or die(mysqli_error($db));  
            $_SESSION['change_success'] = 2;
        }
    }
